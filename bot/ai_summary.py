...
def _normalize_transcript_items(items: Any) -> list[dict[str, Any]]:
    if hasattr(items, "to_raw_data"):
        return items.to_raw_data()
...
def get_youtube_transcript(video_id: str) -> list[dict[str, Any]]:
    ...
    # - старые версии: YouTubeTranscriptApi.get_transcript(...)
    # - новые версии: YouTubeTranscriptApi().fetch(...)
    if hasattr(YouTubeTranscriptApi, "get_transcript"):
        return YouTubeTranscriptApi.get_transcript(video_id, languages=["ru", "en"])  # type: ignore[attr-defined]

    api = YouTubeTranscriptApi()
    fetched = api.fetch(video_id, languages=["ru", "en"])
    return _normalize_transcript_items(fetched)
